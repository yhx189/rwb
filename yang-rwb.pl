...
# height=1024 width=1024 id=\"info\" name=\"info\" onload=\"UpdateMap()\"></iframe>";
  ...
  <label><input type=\"checkbox\" onClick=\"ViewShift()\" name=\"Individuals\" value=\"individuals\">Individuals</label></div>";
  #print checkbox_group(-name=>'showData',
...
if ($action eq "near") {
  ...
  for my $indivCycle (@cycles){
    ...
    if ($what{opinions}) {
      my ($str,$error) = Opinions($latne,$longne,$latsw,$longsw,$indivCycle,$format);
      if (!$error) {
        if ($format eq "table") { 
...
if ($action eq "invite-user") { 
  ...
  print start_form(-method=>'get',
        -action=>'rwb.pl?act=invite-user',
        -enctype=>&CGI::URL_ENCODED);
  print textfield(-name=>'email',
        -value=>'',
        -size=>50,
        -maxlength=>80);
  print submit(-name=>'invite user',
      -value=>'send email');
  print end_form;
